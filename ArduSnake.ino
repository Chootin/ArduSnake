...
const unsigned char PLAYER_BLOCKS[3][4] PROGMEM = {
...
void draw_snakes() {
  for (int i = 0; i < number_of_players; i++) {
    for (int a = 0; a < snakes[i].length; a++) {
      struct Block chunk = snakes[i].chunks[a];
      chunk = snakes[i].chunks[a];
      arduboy.drawBitmap(chunk.x * BLOCK_WIDTH, chunk.y * BLOCK_WIDTH, PLAYER_BLOCKS[i], BLOCK_WIDTH, BLOCK_WIDTH, WHITE);
    }
  }
}
